...
redis 的部署存在四种模式: 单点, [主从](#主从), [哨兵](#哨兵)和[集群](#集群).
...
redis 中的集群只能保持 **AP** 而不能保证强一致性的 **CP**, 其一致性是靠其最终一致性保证的. 因为其数据一致性使用了另外一套机制(异步复制, 异步 failover). 不过也可以使用 `wait` 指令强行同步写, 以实现强一致性.
...
在redis cluster中的分区方式为**哈希槽**，除了该方法还可以使用节点取余，即哈希值对节点数量取余的方式，但该方法在增删节点的时候会导致大部分缓存失效。因此也可以使用一致性哈希分区，即将整个哈希值空间想象成一个环，节点和数据都映射到这个环上。但该方法可能会导致数据分配不均匀。因此最后使用了哈希槽方法。
