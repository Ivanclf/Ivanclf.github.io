...
TCP 是流式协议. 为了提高效率, 发送方可能会把多个数据包合并成一个数据包一起发过去, 于是接收方一次读到多个包. 这种情况叫做'**粘包**; 反之, 发送方一个包很大, 但受限于 MSS(最大报文段大小), 会把它切成好几段发送, 这种情况叫做**拆包**.

为了解决这些问题, 可以强制规定每条消息长度一样, 或者给每条消息末尾加特殊符号. 但现在最常用的是采用**长度域协议**, 即把消息变成 `[消息结构][消息体]` 的结构, 先读长度, 再按长度读取对应字节.

### SCTP

这是另一种传输层协议，集 TCP 的可靠和 UDP 的消息边界于一身。具有以下特性
1. 消息导向： 保留应用层的消息边界，不需要像 TCP 一样给自己加“报文头”来拆粘包。
2. 多流：一条关联里可并行建立多条独立流，单流丢包不会阻塞其他流，从而避免 TCP 的队头阻塞。
3. 多宿主：每个端点可以为同一关联绑定多个 IP 地址。运行时任选一条作为主路径，其余作备份。
4. 相比 TCP，SCTP 提供更高的安全性。
