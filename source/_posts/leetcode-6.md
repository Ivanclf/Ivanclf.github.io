...
{% note success %}
注意二分中中间指针的规范写法应该为`mid = left + (right - left) / 2`，防止数组元素太多，导致`left + right`溢出。不过好像 leetcode 一般不会这么为难人。
{% endnote %}

{% note success %}
令 `L = M` 时, 对应开区间; 令 `L = M + 1` 时, 对应闭区间(即包含 `L` 和 `R` 这两个数). 需要注意这两者的区别, 不要在中途改变其性质.
{% endnote %}
...
{% note info %}
依然是普通的二分查找，只是不同之处在于，如果找到了，需要重置左右指针，再让左右指针向两边扩展，获取范围. 当然, 如果全是闭区间, 也可以直接找 `target + 1` 的上一个值.
...
最简单的办法就是先按照[下一题](#寻找旋转排序数组中的最小值)的思路找到最小值, 然后看看 `target` 是否大于第一个数 / 最后一个数, 然后分左半边或右半边.
