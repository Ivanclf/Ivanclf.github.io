...
4. 客户端不直接使用 new 关键字创建对象，而是调用工厂方法，传入相应参数，工厂返回适当的产品实例给客户端使用
...
这种在类加载时就创建一个实例, 天然线程安全, 但若未使用则会浪费内存
...
在首次使用时创建实例, 通过 `synchronized` 保证线程安全, 但每次获取时都要同步, 性能较低
...
懒加载 + 高效同步, 仅在第一次创建时加锁, 后续直接返回实例即可. 需要 `volatile` 关键字防止指令重排.
