...
|参数名|描述|推荐值范围|适用场景|
|-|-|-|-|
|`temperature`|控制输出的随机性与创造力，值越高随机性越高|0.3~0.7|代码任务控制在 0.1 到 0.3<br>写作任务控制在 0.6 到 0.8|
...
|`num_predict`|本次生成最多输出的 token 数|200~2048|摘要为 200 到 300<br>写文章时为 1024 到 2048<br>写代码时为 512 到 1500|
|`stop`|数组，碰到任意一个字符串就立刻刹车|`["\n", "User:", "问题：", "```"]`|放置模型“自问自答”或者把提示模板也写出来|

要创建自定义模型，需要先创建一个文本文件（如 ModelFile 或 ModelFile.deepseek），然后写入相关参数
