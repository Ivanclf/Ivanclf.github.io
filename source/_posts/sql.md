...
数据库的**数据模型**包括**概念模型**和**逻辑模型/物理模型**。前者按用户的观点建模，用于数据库设计，如 E-R 模型；后者按计算机的观点建模，用于 DBMS 的实现。
...
- 主变量
    SQL 语句中使用的主语言程序变量成为主变量，即 SQL 使用主语言的变量。
- 游标
...
- `varchar` 的最大长度视具体数据库而定。MySQL 中所有列共享 6555 字节，在 UTF8M64 下为 16383 字符；PostgreSQL 中可不指定长度，通常由行最大大小（8kB）限制；SQL Server 中 `varchar(n)` 最大为 8000字符；Oracle 中最大为 4000 字节。
...
- 交并非的操作在不同数据库中的支持不同，非操作在 Oracle 中叫 `minus`。交操作一般是 `intersect`，非操作为 `except`。
