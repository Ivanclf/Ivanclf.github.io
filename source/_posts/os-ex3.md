...
上一篇说到，对于客户端卡在 `fgets`，感知不到服务器断开的问题，可以使用某些函数来解决。以下将讲清楚这些函数的使用方式。这些函数的核心作用是，让进程同时监听多个 FD，仅当任意一个 FD 有事件时才唤醒线程，这样，既不会漏了键盘输入，也不会漏了断开事件。

以 `select` 为例。当服务器被 `kill -9` 后，内核会主动给客户端发 FIN，客户端内核将该 `sockfd` 标记为“可读”，然后 `select` 检测到 `sockfd` 有读事件，唤醒进程，于是进程不再卡在 `fgets` 而是优先处理套接字事件。
...
1. 切换/调度开销极大。在调度时需要做上下文切换，每次切换时耗时大，在高并发场景下会被无限放大。
2. 内存开销高。每个线程会默认分配几 MB 的空间（Linux 下默认为 8MB），在多个线程时占用空间巨大。
3. 线程是抢占式调度，操作系统随时可能打断线程。多个线程操作共享数据时容易产生死锁等问题。
4. 实际业务中，大部分并发场景都是 IO 密集型。线程在执行这些任务时，绝大部分时间都在等待，但线程在等待时，仍然会占用资源而不会主动释放。

## 协程的概念

协程是用户态的轻量级的线程，可以通俗理解为“可以暂停和恢复的函数”。
1. 其切换开销极小，仅需要保存函数暂停点。
2. 内存开销低，由于其可以看作是一种函数，因此它占用的是栈空间，初始仅几 KB。
3. 协程是非抢占式，协作式调度。
4. 对于 IO 密集型任务，协程在遇到等待时会主动让出 CPU，让其他协程执行。
