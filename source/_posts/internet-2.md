...
2. 浏览器通过 **DNS** 协议，获取域名对应的 IP 地址
    DNS 在查询和返回时都是用的是 UDP 协议, 端口 53. 只有在某些特殊情况下会使用 TCP 协议
3. 浏览器根据 IP 地址和端口号，向目标服务器发起一个 **TCP**链接请求
    如果多个域名指向同一个 IP, 那么 HTTP 协议中的 Host 请求头部分会进行区分, 该部分会写上请求域名. Web 服务器根据这个 Host 找到对应的虚拟主机, 返回对应网站的内容. 如果是 HTTPS, 那么还会用到 SNI, 在 TLS 握手阶段将域名传给服务器, 让服务器选对证书.
...
6. 浏览器收到 HTTP 响应报文后，解析响应体中的 HTML 代码，渲染网页的结构和样式。同时根据 HTML 中的其他内容的 URL （如 CSS 或 JS 文件），再次发起 HTTP 请求，直到网页完全加载
    但现代的浏览器支持并行请求, 多路复用, 可以同步请求 HTML 和 CSS 文件.
...
在浏览器链接到服务器的 443 端口后, 其握手过程(TLS 1.2)如下
1. Client Hello: 客户端发送支持的 TLS 版本, 密码套件列表, 随机数 **Client Random**.
2. Server Hello: 服务器选择协议版本和密码套件, 并发送另一个随机数 **Server Random**.
3. 证书: 服务器发送数字证书, 这其中包含一个公钥.
4. 密钥交换: 客户端验证证书后, 生成一个随机生成的**预主密钥**(Pre-Master Secret), 用服务器公钥加密后发送
5. 计算会话密钥: 双方根据 Client Random, Server Random 和 Pre-Master Secret 独立计算出相同的会话密钥.
6. 握手完成. 客户端和服务器分别通知对方后续通信将加密, 并发送验证信息.
...
HTTP/1.0 的核心特点即为短链接，每次请求都需要重新建立一次 TCP 链接，收到响应后立即关闭链接。该方式的性能极差，开销巨大，并且会造成**队头阻塞**：上一个请求响应慢了，会阻塞后面所有请求。
...
并且 HTTP/1.1 也有内容上的差别。HTTP/1.1 中加入了大量的状态码，如 `100` (请求大资源前的预热请求)、`409` (请求和当前资源的规定冲突) 等。
...
注意，WebSocket 需要先通过 HTTP Upgrade 握手建立链接，然后才切换到 WS 通信协议。
...
{% note info %}
网关往往会有同一封装结果类的操作，但需要注意不要给 WS 或者 SSE 协议的数据操作，因为二者都是持续推送模型，而结果类封装适合于一次返回完成的操作，二者语义不匹配。
{% endnote %}
...
CSRF 通常是跨域的，因为外域通常更容易被攻击者掌控。浏览器通常规定了**同源策略**：允许跨域发送请求，阻止跨域读取响应。这刚好契合了 CSRF 的需求。
...
这是一种基于无状态 (如 JWT) 的 API 简化方案。用户登录后，服务器会给一个随机的 Cookie，这个 Cookie 对前端 JS 是可读的。前端在发起任何非幂等请求时，必须从 Cookie 中读取这个值，并将其作为自定义请求头或请求体字段再次发送。服务端比较请求头/体中的 Token 值是否和 Cookie 的值一致。
