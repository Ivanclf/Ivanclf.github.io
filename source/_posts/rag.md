...
一般使用按 token 分块, 并重叠一定量 token 的方案, 主要是处于以下考量
1. 句子长度可能不太可控. token 的长度是一定可控的, 但不同文档的分隔符可能不一样, 那么当分隔符比较逆天(比如就没有分隔符, 人家打空格当句号的)时, 一句的长度可能就比较难看了. 按照 token 分块则没有这个问题.
2. 重叠一定量的 token 主要是避免跨 chunk 语义断裂的问题, 当用户的 prompt 的关键词刚好卡在边界情况时, 向量库中就能同时响应两片分片, 从而缓解 chunk 语义断裂的问题.
